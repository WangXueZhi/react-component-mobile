...
import { Popup, tools } from 'wd-mobile'
...
export default class Picker extends React.PureComponent<PickerProps, any> {
  ...
  componentDidUpdate(_prevProps: Readonly<PickerProps>, _prevState: Readonly<any>, _snapshot?: any): void {}
          ...
          <div className={`${Picker.defaultProps.prefixCls}-multiple-container`}>
            {data.map((item: any, index) => (
              <Picker
                ...
                onChange={v => onChange && onChange({ ...v, col: index }, sender)}
                {...resetProps}
              />
            ))}
          </div>
...
export interface PickerProps extends React.InputHTMLAttributes<{}> {
   ...
   * 滚动阶段，没到一个区间触发一次
   * */
  onChange?: (item: PickerDataType, sender: Picker) => any
}

// 数据源格式
export type PickerDataType = { label: any, value?: any, children?: PickerDataType[], [key: string]: any }