...
import { Toast, tools } from 'wd-mobile'
...
export default class Form extends React.PureComponent<FormProps, any> {
   ...
   * */
  reset() {
    this.inputs.forEach(input => input.reset && input.reset())
  }
  ...
  checkValidity() {
    return this.inputs.map(input => input.checkValidity
      ? { ...input.checkValidity(), vNode: input }
      : { valid: true }
    )
  }
  ...
  handleCloneChildren(children: any): any {
    ...
    if (children && children.props) {
      ...
      if ((React.isValidElement(children.props.children) || Array.isArray(children.props.children)) && !children.props.name && !children.props.__type__) {
