...
export default class Popup extends React.PureComponent<PopupProps, any> {
  ...
  componentDidUpdate(_prevProps: Readonly<PopupProps>, _prevState: Readonly<any>, _snapshot?: any): void {}

  render() {
    const { prefixCls, className, children, visible, mask, maskStyle, onClickMask, offset: _offset, ...resetProps } = this.props
    ...
    return (
      <div className={cls(prefixCls, className, { [`${prefixCls}-visible`]: visible })} {...resetProps}>
        ...
        >{children}</div>
        {mask
          ? <div style={maskStyle} ref={this.refPopupMask} onClick={onClickMask} className={`${prefixCls}-mask`} />
          : null
        }
      </div>
    )
